...
def get_order_flow_module():
    from open_flipkart_active_orders import (
        DEFAULT_PROFILE_NAME,
        TARGET_URL,
        build_firefox_driver,
        log_event,
        prompt_for_firefox_profile,
    )

    return {
        "DEFAULT_PROFILE_NAME": DEFAULT_PROFILE_NAME,
        "TARGET_URL": TARGET_URL,
        "build_firefox_driver": build_firefox_driver,
        "log_event": log_event,
        "prompt_for_firefox_profile": prompt_for_firefox_profile,
    }
...
def perform_login_check(
    ...
    *,
    ...
    helpers = get_order_flow_module()
    log_event = helpers["log_event"]
...
def check_logged_in(
    ...
    *,
    ...
    helpers = get_order_flow_module()
    build_firefox_driver = helpers["build_firefox_driver"]
    driver = build_firefox_driver(profile_name)

    try:
        return perform_login_check(
            driver,
            target_url,
            profile_name,
...
def run_login_check_for_profile(
    target_url: str,
    profile_name: str,
    *,
    prompt_after_gmail: bool = True,
...
def main() -> None:
    helpers = get_order_flow_module()
    prompt_for_profile = helpers["prompt_for_firefox_profile"]
    default_target_url = helpers["TARGET_URL"]
    ...
    if cli_target_url is None:
        target_url = prompt_with_default("Enter desired URL", default_target_url)
