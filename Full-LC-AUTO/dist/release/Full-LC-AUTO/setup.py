...
PACKAGE = ROOT / "setup-package.json"
...
class App:
    ...
    def build_ui(self) -> None:
        ...
        ttk.Button(actions, text="Import Setup Package", command=self.import_setup_package).pack(side="left")
        ttk.Button(actions, text="Export Setup Package", command=self.export_setup_package).pack(side="left", padx=(10, 0))
        ttk.Button(actions, text="Preview Output", command=self.preview).pack(side="left", padx=(10, 0))
    ...
    def clear_form(self) -> None:
        ...
        self.vertical_rows.clear()

    def collect_setup_package(self) -> dict[str, object]:
        return {
            "version": 1,
            "exported_at": datetime.now().isoformat(timespec="seconds"),
            "setup": {
                "laptop_name": self.laptop.get().strip().upper() or "NEW_LAPTOP",
                "snapshot_directory": self.snapshot_dir.get().strip() or "snapshots",
                "project_paths": {
                    "run_helpers_directory": self.run_helpers.get().strip() or "run_helpers",
                    "error_latest_path": self.error_file.get().strip() or "run_helpers/error_latest.txt",
                    "success_run_record_path": self.success_record.get().strip() or "successful-run-record.xlsx",
                    "flow_config_root": self.flow_root.get().strip() or "json_LC_creation",
                    "data_inputs_root": self.data_inputs_root.get().strip() or "data inputs",
                    "assets_root": self.assets_root.get().strip() or "assets",
                },
    ...
    def load_setup_package_payload(self, payload: dict[str, object]) -> None:
        setup = payload.get("setup", {})
        if not isinstance(setup, dict):
            raise ValueError("Setup package is missing the setup block.")
        self.clear_form()
        self.laptop.set(str(setup.get("laptop_name", "NEW_LAPTOP")))
        self.snapshot_dir.set(str(setup.get("snapshot_directory", "snapshots")))
        project_paths = setup.get("project_paths", {})
        if isinstance(project_paths, dict):
            self.run_helpers.set(str(project_paths.get("run_helpers_directory", self.run_helpers.get())))
            self.error_file.set(str(project_paths.get("error_latest_path", self.error_file.get())))
            self.success_record.set(str(project_paths.get("success_run_record_path", self.success_record.get())))
            self.flow_root.set(str(project_paths.get("flow_config_root", self.flow_root.get())))
            self.data_inputs_root.set(str(project_paths.get("data_inputs_root", self.data_inputs_root.get())))
            self.assets_root.set(str(project_paths.get("assets_root", self.assets_root.get())))
        ...
        if isinstance(accounts, list) and accounts:
            for account in accounts:
                if not isinstance(account, dict):
                    continue
                brands = []
                for brand in account.get("brands", []):
                    if isinstance(brand, dict):
                        brands.append((str(brand.get("name", "")), str(brand.get("code", ""))))
                self.add_account(str(account.get("profile", "")), str(account.get("alias", "")), str(account.get("firefox_profile", "")), brands or None)
        else:
            self.add_account()
        verticals = setup.get("verticals", [])
        if isinstance(verticals, list) and verticals:
            for vertical in verticals:
                if not isinstance(vertical, dict):
                    continue
                kinds = []
                for kind in vertical.get("kinds", []):
                    if isinstance(kind, dict):
                        kinds.append((str(kind.get("kind", "")), str(kind.get("image_directory", ""))))
                self.add_vertical(str(vertical.get("name", "")), str(vertical.get("default_kind", "")), str(vertical.get("default_size", "28")), kinds or None, str(vertical.get("scaffold_source", "auto")))
        else:
            self.add_vertical()
        self.refresh_statuses()

    def export_setup_package(self) -> None:
        try:
            payload = self.collect_setup_package()
        except Exception as exc:
            messagebox.showerror("Export failed", str(exc), parent=self.root)
            return
        target = filedialog.asksaveasfilename(parent=self.root, title="Export setup package", defaultextension=".json", initialfile=PACKAGE.name, filetypes=[("JSON files", "*.json")])
        if not target:
            return
        Path(target).write_text(json.dumps(payload, indent=2), encoding="utf-8")
        messagebox.showinfo("Setup package exported", str(target), parent=self.root)

    def import_setup_package(self) -> None:
        target = filedialog.askopenfilename(parent=self.root, title="Import setup package", filetypes=[("JSON files", "*.json")])
        if not target:
            return
        try:
            payload = json.loads(Path(target).read_text(encoding="utf-8"))
            self.load_setup_package_payload(payload)
        except Exception as exc:
            messagebox.showerror("Import failed", str(exc), parent=self.root)
            return
        messagebox.showinfo("Setup package imported", str(target), parent=self.root)
    ...
    def generate(self) -> None:
        try:
            ...
            return
        package_payload = self.collect_setup_package()
        DEMO.write_text(json.dumps(output, indent=2), encoding="utf-8")
        PACKAGE.write_text(json.dumps(package_payload, indent=2), encoding="utf-8")
