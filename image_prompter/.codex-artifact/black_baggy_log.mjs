...
const previewPath = "C:/Users/ESHAAN/HAKUR/work--fk-tools/image_prompter/.codex-artifact/black-baggy-log-preview.png";
...
const match = values.map((row, index) => ({ row: index + 1, values: row }))
  .find(({ values: row }) => String(row[0] ?? "").toUpperCase() === "BLACK-BAGGY");

if (!match) throw new Error("Black Baggy row not found");

if (mode === "inspect") {
  const styles = await workbook.inspect({
    kind: "computedStyle",
    sheetId: sheet.name,
    range: `A${match.row}:CR${match.row}`,
    maxChars: 3000,
  });
  const preview = await workbook.render({ sheetName: sheet.name, autoCrop: "all", scale: 1, format: "png" });
  await fs.writeFile(previewPath, new Uint8Array(await preview.arrayBuffer()));
  console.log(JSON.stringify({ sheet: sheet.name, row: match.row, values: match.values, styles: styles.ndjson, previewPath }, null, 2));
} else if (mode === "update") {
  const concepts = JSON.parse(process.argv[3]);
  const firstEmpty = match.values.findIndex((value, index) => index > 0 && (value === null || value === ""));
  if (firstEmpty < 0) throw new Error("No empty cells available in Black Baggy row");
